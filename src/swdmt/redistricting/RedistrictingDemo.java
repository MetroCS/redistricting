...
/**
 * Class to demo the usage of the Redistricting Library.
 *
 * @author Dr. Jody Paul
 * @version 20191130
 */
public final class RedistrictingDemo {
    ...
    /**
     * Main driver for redistricting application demo.
     * 
     * @param args ignored
     */
    public static void main(final String[] args) {
        int rows = DEFAULT_NUM_ROWS, columns = DEFAULT_NUM_COLUMNS, numLocations = DEFAULT_NUM_LOCATIONS,
                numDistricts = DEFAULT_NUM_DISTRICTS;
        rows = Integer.parseInt(askUser("Please enter the number of rows "));
        columns = Integer.parseInt(askUser("Please enter the number of columns "));
        numDistricts = Integer.parseInt(askUser("Please enter the number of districts "));
        ...
        System.out.println("Created region " + workingRegion + " with " + numLocations + " locations");
        ...
        for (Location loc : workingRegion.locations()) {
            voterSet.add(new Voter(rng.nextInt(2) == 0 ? Party.PARTY0 : Party.PARTY1, loc));
        }
        // Create region with specified voters
        Region region = new Region((Set<Location>) workingRegion.locations(), voterSet);
        System.out.println("Assigned voters with arbitrary parties to locations.");
        System.out.println(Renderer.renderAsASCII(true, region));
        Set<District> districts = Redistrictor.generateDistricts(region, numDistricts);
